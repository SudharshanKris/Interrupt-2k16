...
$post_ans=$_POST['answer'];
$post_ans = str_replace(' ', '', $post_ans);
$ur_ans = $_SESSION['answer'][$_SESSION['x']];
$ur_ans = str_replace(' ','',$ur_ans);
    if(strtolower(trim($post_ans)) == strtolower(trim($ur_ans)))
    {
...
$query_Recordset3="UPDATE  `connection`.`conn_user` SET  `clevel` =  `clevel`+1, `updated_at` = now() WHERE  `conn_user`.`id` ='$user_id'";
        // echo $query_Recordset3;
            $Recordset3 = mysqli_query($sql,$query_Recordset3);
$query_Recordset3="UPDATE  `connection`.`conn_user` SET  `cques` =  0, `updated_at` = now() WHERE  `conn_user`.`id` ='$user_id'";
        // echo $query_Recordset3;
            $Recordset3 = mysqli_query($sql,$query_Recordset3);
...
{
echo "That's all!!! Keep tuning for SVCE Interrupt FB pages for updates... ";
...
<!-- onload="restrainIllegalOperation()" -->
